    ...
    private OthelloClient() {
        ...
        button_save.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                WriteFile w = new WriteFile(input, oxBoardc.board, oxBoardc.text, oxBoardc.black, oxBoardc.white, oxBoardc.turn);
            }
        });
        ...
        button_set.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                label.setText(input);
                //label.setText(input);
                oxBoardc.currentNumber = Integer.parseInt(input);
                //oxBoard.countdown();
            }
        });
        ...
        try{
        oxBoardc.CountdownPresenterPanel(label);}
        catch(Exception e3){}
        panelTop.add(text);
        panelTop.add(label);
...
class OXC extends Component implements MouseListener, MouseMotionListener, Runnable {
    public int[] board; // 盤面狀況,表達有邊框的10*10盤面
    public int turn, diskdiff; // 現在哪方可下, 與敵方的子數差異
    public String[] text;
    public int black, white;
    public int movetimes;   
    private OXC parent; // 由哪一個盤面變化而來
    ...
    public boolean equals(Object o) {
        if (!(o instanceof OXC)) return false;
        OXC t = (OXC) o;
        for (int i=11; i<89; i++) {
